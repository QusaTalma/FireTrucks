...
public class AkUnityPluginInstaller_Android : AkUnityPluginInstallerBase
{
	...
	{
		m_platform = "Android";
		m_arches = new string[] { "armeabi-v7a" };
	}
...
public class AkUnityIntegrationBuilder_Android : AkUnityIntegrationBuilderBase
{
	...
	protected override bool PreBuild()
	{
		...
		if ( fi.Exists )
		{
			string msg = string.Format("Wwise: Use build variables defined in preference file: {0}. Edit or delete the file by hand if needed.", configPath);
			UnityEngine.Debug.Log(msg);
		}
		else 
		{
			const string AbortMessage = "Wwise: User cancelled the build.";

			string msg = string.Format("Wwise: Need user input because preference file is unavailable at: {0}.", configPath);
			UnityEngine.Debug.Log(msg);
